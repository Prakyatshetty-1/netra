...
def _fetch_labels(nodes: dict[str, dict[str, Any]]) -> None:
    ...
    if persons:
        ...
        for row in query(
            f"""
            SELECT PersonID, FullName, SourceRole
            FROM Person
    ...
    if accounts:
        ...
        for row in query(
            f"""
            SELECT AccountID, AccountNumber, BankName
            FROM BankAccount
    ...
    if vehicles:
        ...
        for row in query(
            f"""
            SELECT VehicleID, RegistrationNumber, VehicleType
    ...
    if group_ids:
        ...
        for row in query(
            f"""
            SELECT GroupID, EventDescription
    ...
    for n in nodes.values():
        if (
            ...
            n["label"] = (
                "Incident scene"
                if n["entity_id"] == 0
                else n.get("label") or n["id"]
            )


def fetch_case_edges(case_id: int) -> list[dict[str, Any]]:
    """Fetch relationship edges belonging to a case."""
...
def collect_nodes(
    ...
    """Collect nodes referenced by graph edges."""
...
def collect_case_person_nodes(
    ...
    """
    Add every person belonging to the case.

    This is important for cases that have persons recorded but
    currently have no GraphEdge rows.

    Example:
        Case 69
            Person 69  -> Suresh Gowda
            Person 309 -> Shivakumar Desai
            Person 189 -> Harish Hegde

    Even when the case has zero edges, these people should still
    ...
    for row in rows:
        ...
        if key not in nodes:
            ...
            # Preserve any information already collected from
            # GraphEdge while making sure the person has a label.
            nodes[key]["label"] = row["FullName"]
            nodes[key]["role"] = row["SourceRole"]
...
def build_nx_graph(
    ...
    """
    Build an undirected simple graph used for centrality,
    communities and counterfactual analysis.
    """
...
def case_graph(
    ...
    """
    Build the complete graph representation for a case.

    Important:
    Persons are included even when the case has zero edges.
    """

    # 1. Fetch existing relationships.
    edges = fetch_case_edges(case_id)

    # 2. Build nodes from relationships.
    nodes = collect_nodes(edges)

    # 3. Add all persons belonging to this case.
    #
    #    This fixes cases such as Case 69 where:
    #
    #       person_count = 3
    #       edge_count   = 0
    #
    ...
    #       nodes = {}
    #
    #    Now the three persons will still appear.
    collect_case_person_nodes(case_id, nodes)

    # 4. Re-fetch labels for any newly added nodes.
    _fetch_labels(nodes)
...
def highlight_from_name(
    ...
    for e in payload["edges"]:
        e["highlighted"] = (
            e["source"] in reachable
            and e["target"] in reachable
        )
